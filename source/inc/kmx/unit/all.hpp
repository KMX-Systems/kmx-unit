/// @copyright Copyright (c) 2025 - present KMX Systems. All rights reserved.
/// @file inc/kmx/unit/all.hpp
/// @brief Includes every unit family of the library. A translation unit needing one family only is better
/// served by the header of that family.
#pragma once
#ifndef PCH
    #include <kmx/unit/acceleration.hpp>
    #include <kmx/unit/amount.hpp>
    #include <kmx/unit/angle.hpp>
    #include <kmx/unit/angular_velocity.hpp>
    #include <kmx/unit/area.hpp>
    #include <kmx/unit/base.hpp>
    #include <kmx/unit/chrono.hpp>
    #include <kmx/unit/data_rate.hpp>
    #include <kmx/unit/distance.hpp>
    #include <kmx/unit/duration.hpp>
    #include <kmx/unit/electromagnetism.hpp>
    #include <kmx/unit/energy.hpp>
    #include <kmx/unit/force.hpp>
    #include <kmx/unit/frequency.hpp>
    #include <kmx/unit/illuminance.hpp>
    #include <kmx/unit/information.hpp>
    #include <kmx/unit/luminous_intensity.hpp>
    #include <kmx/unit/mass.hpp>
    #include <kmx/unit/math.hpp>
    #include <kmx/unit/power.hpp>
    #include <kmx/unit/pressure.hpp>
    #include <kmx/unit/solid_angle.hpp>
    #include <kmx/unit/temperature.hpp>
    #include <kmx/unit/text.hpp>
    #include <kmx/unit/torque.hpp>
    #include <kmx/unit/velocity.hpp>
    #include <kmx/unit/version.hpp>
    #include <kmx/unit/volume.hpp>
#endif
